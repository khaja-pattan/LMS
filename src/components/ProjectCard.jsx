...
import { ArrowUpRight, Coffee, Terminal, Film, Cpu, Globe } from 'lucide-react';
...
const ProjectCard = ({ project }) => {
  ...
  const theme = getTheme(project.tech);
  ...
  return (
    <motion.div
      ...
      className="group relative rounded-[32px] border border-slate-200/50 dark:border-slate-800/50 bg-slate-100/40 dark:bg-slate-900/40 backdrop-blur-xl overflow-hidden cursor-pointer shadow-sm"
    >
      <div className="h-44 w-full relative overflow-hidden">
        ...
        <div className={`absolute top-4 left-4 p-2.5 rounded-xl backdrop-blur-md ${theme.bg} ${theme.color} border border-white/10 shadow-lg`}>
          {theme.icon}
        </div>
      </div>
      <div className="p-7">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6 font-medium line-clamp-1">{project.tech}</p>
        <div className="pt-5 border-t border-slate-200/20 flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
